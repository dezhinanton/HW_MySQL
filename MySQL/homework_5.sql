...
DROP VIEW users_with_city;

CREATE VIEW users_with_city AS
...
SELECT
DENSE_RANK() OVER(ORDER BY count_messages DESC) AS rank_on_message,
firstname, lastname, count_messages
FROM
(SELECT from_user_id, firstname, lastname, COUNT(from_user_id) AS count_messages
FROM users as u
JOIN messages as m
ON u.id = m.from_user_id
GROUP BY from_user_id) AS messages_from_users;
 ...
 */
 ...
 SELECT 
 body, created_at, next_time, TIME(next_time-created_at) AS sending_interval
 FROM
 (SELECT 
 body, created_at,
 LEAD(created_at) 
 OVER w AS next_time
 FROM messages 
 WINDOW w AS (ORDER BY created_at)) AS interval_table;
