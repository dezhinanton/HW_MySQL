...
DELIMITER //
CREATE PROCEDURE transfer(IN id_need BIGINT)
BEGIN
START TRANSACTION;
INSERT INTO users_old (id, firstname, lastname, email)
SELECT id, firstname, lastname, email FROM users
WHERE id = id_need;
DELETE FROM users WHERE id = id_need;
COMMIT;
END//
DELIMITER ;
...
DELIMITER //
CREATE FUNCTION  hello(user_name VARCHAR(50))
...
SELECT 
	CASE 
		...
		ELSE "Добрый вечер"
	END INTO greetings;
RETURN CONCAT(greetings,", ",user_name);	
END//
DELIMITER ;
