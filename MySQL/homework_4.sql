...
SELECT p.birthday, TIMESTAMPDIFF(YEAR, p.birthday, NOW()) AS age, COUNT(l.user_id)  AS count_likes
FROM likes AS l, `profiles` AS p
WHERE l.user_id = p.user_id
GROUP BY l.user_id
having age < 12;

-- 2. Определить кто больше поставил лайков (всего): мужчины или женщины. 
SELECT p.gender, COUNT(l.user_id)  AS count_likes
FROM likes AS l, `profiles` AS p
WHERE l.user_id = p.user_id
GROUP BY p.gender;

-- 3. Вывести всех пользователей, которые не отправляли сообщения.
SELECT CONCAT(users.id, " ", users.firstname, " ", users.lastname) AS person, COUNT(messages.from_user_id) AS count_send_message
FROM users
LEFT JOIN messages ON messages.from_user_id = users.id
GROUP BY person
HAVING count_send_message = 0;

-- 4. (по желанию)* Пусть задан некоторый пользователь (id = 1). Из всех друзей этого пользователя найдите человека, который больше всех написал ему сообщений.
SELECT CONCAT(users.firstname, " ", users.lastname) AS "Отправитель", COUNT(messages.from_user_id) AS count_message
FROM messages, users
WHERE messages.to_user_id = 1 and messages.from_user_id = users.id
